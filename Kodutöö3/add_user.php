<?php
  require_once "../../../conf.php"; //andmebaasi info
  require_once "fnc_general.php"; //valideerimis info
  require_once "fnc_user.php"; //Fail kus on kirjas kõik kasutajaga loomisega seotud funktsioonid
  ...
  if(isset($_POST["user_data_submit"])){
	...
	if(!empty($_POST["birth_day_input"])){ //juhul kui birthday input pole tühi
		$birth_day = intval($_POST["birth_day_input"]); //saab muutjua birthday input antud väärtuse (intval teeb täisarvu)
	} else {
		$birth_day_error = "Palun vali sünnikuupäev!"; //kui birthday input tühi siis saab antud vastuse
	}
	
	if(!empty($_POST["birth_month_input"])){ //juhul kui siüünikuu input pole tühi
		$birth_month = intval($_POST["birth_month_input"]); //saab muutuja birth_month antud väärtuse (kuud on kodeeritud numbriteks)
	} else {
	...
	if(empty($birth_day_error) and empty($birth_month_error) and empty($birth_year_error)){
		if(checkdate($birth_month, $birth_day, $birth_year)){//checkdate funktsioon kontrollib kas reaalselt antud kuupäev on olemas.
	...
	if(!empty($_POST["password_input"])){ //kui parooli input pole tühi
		if(strlen($_POST["password_input"])<8){ //paroolipikkuse nõue
