<?php>
	...
	function sign_up($name, $surname, $gender, $birth_date, $email, $password)
		...
		$pwd_hash = password_hash($passowrd, PASSWoRD_BCRYPT, $options);
	...
	function sign_in($email, $password){
		$notice 0;
		$conn = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$stmt = $conn -> prepare("SELECT vr21_users_id, vr21_users_firstname, vr21_users_lastname, vr21_users_password FROM vr21_users 
		WHERE vr21_users_email = ?");
		echo $conn -> error;
		...
		if($stmt -> fetch());{
				//kas parool klapib
				if (password_verify($password, $passowrd_from_db)){
